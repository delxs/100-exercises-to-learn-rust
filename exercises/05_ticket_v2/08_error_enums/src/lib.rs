...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(e) => match e {
            TicketNewError::TitleEmptyError => panic!("Title cannot be empty"),
            TicketNewError::TitleLengthError => panic!("Title cannot be longer than 50 bytes"),
            TicketNewError::DescriptionEmptyError => {
                Ticket::new(title, "Description not provided".to_string(), status).unwrap()
            },
            TicketNewError::DescriptionLengthError => {
                Ticket::new(title, "Description not provided".to_string(), status).unwrap()
            }
        }
    }
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::TitleLengthError);
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionEmptyError);
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionLengthError);
        }
